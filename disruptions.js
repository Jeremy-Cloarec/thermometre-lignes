...
function processDisruptionData(data) {
    ...
    async function generateCard() {
        for (const disruption of disruptions) {
            ...
            //Netttoyage du message
            let messageToClean = disruption.messages.text;
            console.log(messageToClean);

            const elevation = document.createElement("div");
            elevation.classList.add("elevation", "hidden");
            elevation.setAttribute("data-iv", "number");

            const messageContainer = document.createElement("div");
            messageContainer.classList.add("message-container");

            messageContainer.innerHTML = messageToClean;

            elevation.appendChild(messageContainer);
            containerDisruption.appendChild(elevation);
    ...
    async function clickToOpen() {
        const openDisruption = document.querySelectorAll('#open-disruption')

        openDisruption.forEach( (openDisruption) => {
